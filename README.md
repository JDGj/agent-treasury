...
**No dependencies.** Python 3.9+, standard library only. Four files, each with
...
`record()` refuses a negative amount with no category. Fail closed. Factual
movements — the processor's own fees, a scheduled penalty, the seed capital —
are recording reality and are exempt, because a ledger that refuses to write
down what already happened is worse than no ledger.
...
| `redact.py` | Scrubs credential-shaped strings out of anything the agent is about to publish or send. |

Each runs its own tests with no network and no fixtures:
